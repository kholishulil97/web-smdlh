...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        $('#tabelSP').DataTable({
            ...
            'ajax': {
                'url': '<?= base_url() ?>controllerkasir/daftarpembayaranpremi'
            },
            ...
            buttons: [{
                    ...
                    title: 'Daftar Pembayaran Premi Kru Jalan PT. Akas Mila Sejahtera'
                },
                {
                    extend: 'pdfHtml5',
                    download: 'open',
                    ...
                    title: 'Daftar Pembayaran Premi PT. Akas Mila Sejahtera',

                },
                {
                    extend: 'print',
                    messageTop: 'Daftar Pembayaran Premi PT. Akas Mila Sejahtera, Probolinggo.',
                    exportOptions: {
                        columns: ':visible'
                    }
                },
    ...
    function setor(event, id) { //ketika menekan tombol hapus maka akan muncul modal sweetalert
        event.preventDefault();
        var url = document.getElementById("foto" + id).getAttribute("href");
        var nominal = $('[name="sweet_nominal' + id + '"]').val();
        var nominal_simpan = $('[name="sweet_nominal_simpan' + id + '"]').val();
        var setor = $('[name="sweet_setor' + id + '"]').val();
        var kru_id = $('[name="sweet_kru_id' + id + '"]').val();
        Swal.fire({
            title: 'Peringatan!',
            ...
            if (result.value) {
                $.ajax({
                    url: "<?php echo site_url('controllerkasir/bayarpremi') ?>",
                    type: "POST",
                    dataType: "JSON",
                    data: {
                        id: kru_id,
                        setor: setor,
                        nominal: nominal_simpan
                    },
                    success: function(data) {
                        ...
                        Swal.fire({
                            title: "Berhasil!",
                            text: "Berhasil mengupdate data pembayaran premi",
                            icon: 'success',
                            scrollbarPadding: false
                        });
                    },
